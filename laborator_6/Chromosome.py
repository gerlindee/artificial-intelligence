...
DEPTH_MAX = 5
terminals = ['No', 'Cement', 'Slag', 'Fly_ash', 'Water', 'SP', 'Coarse_Aggr', 'Fine_Aggr', 'SLUMP', 'FLOW'] + ['c' + str(i) for i in range(3)]
noTerminals = 10
functions = ['+', '-', '*']
...
class Chromosome:
    def __init__(self, depth=DEPTH_MAX):
        self.maxDepth = depth
        self.repres = [0 for i in range(2**(self.maxDepth + 1) - 1)]
        self.growExpression()
        self.fitness = 0
        self.size = 0

    def growExpression(self, pos=0, depth=0):
        # initialize randomly an expression
    ...
    def computeFitness(self, crtData, crtOut, noExamples):
        ...
        for d in range(noExamples):
            err = err + abs(crtOut[d] - self.evalExpression(0, crtData[d][0]))
        self.fitness = err / len(crtData)**2
        return self.fitness

    def traverse(self, pos):
        # returns the next index where the next branch in the tree, on the same level, begins
